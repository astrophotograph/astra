    ...
    try:
        ...
        for obj in objects:
            ...
            try:
                ...
                # For TOP-DOWN FITS (SharpCap), the preview image generated by
                # processinator has its X-axis mirrored relative to the WCS
                # pixel coordinates from solve-field. This is because the FITS
                # stores data as (3,H,W) with a different X convention than
                # standard FITS bottom-up layout. Flip X only.
                if is_top_down:
                    px = naxis1 - 1 - px
