    ...
    try:
        ...
        for obj in objects:
            ...
            try:
                ...
                # For TOP-DOWN FITS (SharpCap), the pixel storage order is
                # inverted relative to standard FITS convention. The WCS was
                # solved against the raw pixel layout, but the preview image
                # is generated by processinator which transposes (3,H,W) to
                # (H,W,3) — this preserves the storage order. However, the
                # WCS assumes standard FITS coordinates. We need to flip both
                # axes to map from WCS pixel coords to preview pixel coords.
                if is_top_down:
                    px = naxis1 - 1 - px
                    py = naxis2 - 1 - py
