...
const EQUIPMENT_PRESETS: EquipmentPreset[] = [
  ...
  },
  {
    ...
    telescope: {
      name: "Seestar S30 Pro APO",
      aperture: "30",
      focalLength: "160",
      type: "Quadruplet APO (ED)",
    },
...
export default function AdminPage() {
  ...
  const handleSaveLocation = () => {
    ...
    if (editingLocationId) {
      updateLocation(editingLocationId, {
        ...
        horizon,
        equipmentIds:
          locationForm.equipmentIds.length > 0
            ? locationForm.equipmentIds
            : undefined,
      });
  ...
  const handleSaveEquipment = () => {
    ...
    const telescope: TelescopeType | undefined =
      equipmentForm.telescope.name.trim()
        ? {
            name: equipmentForm.telescope.name,
            aperture: equipmentForm.telescope.aperture
              ? parseFloat(equipmentForm.telescope.aperture)
              : undefined,
            focalLength: equipmentForm.telescope.focalLength
              ? parseFloat(equipmentForm.telescope.focalLength)
              : undefined,
    ...
    const camera: CameraType | undefined = equipmentForm.camera.name.trim()
      ? {
          ...
          sensorWidth: equipmentForm.camera.sensorWidth
            ? parseFloat(equipmentForm.camera.sensorWidth)
            : undefined,
          sensorHeight: equipmentForm.camera.sensorHeight
            ? parseFloat(equipmentForm.camera.sensorHeight)
            : undefined,
          pixelSize: equipmentForm.camera.pixelSize
            ? parseFloat(equipmentForm.camera.pixelSize)
            : undefined,
    ...
    const guideScope: GuideScopeType | undefined =
      equipmentForm.guideScope.name.trim()
        ? {
            name: equipmentForm.guideScope.name,
            aperture: equipmentForm.guideScope.aperture
              ? parseFloat(equipmentForm.guideScope.aperture)
              : undefined,
            focalLength: equipmentForm.guideScope.focalLength
              ? parseFloat(equipmentForm.guideScope.focalLength)
              : undefined,
          }
        : undefined;

    // Build guide camera object if any fields are filled
    const guideCamera: GuideCameraType | undefined =
      equipmentForm.guideCamera.name.trim()
        ? {
            name: equipmentForm.guideCamera.name,
            pixelSize: equipmentForm.guideCamera.pixelSize
              ? parseFloat(equipmentForm.guideCamera.pixelSize)
              : undefined,
          }
        : undefined;
  ...
  const handleDeleteEquipment = (id: string) => {
    ...
    const usedByLocations = locations.filter((loc) =>
      loc.equipmentIds?.includes(id),
    );
    if (usedByLocations.length > 0) {
      toast.error(
        `This equipment is used by ${usedByLocations.length} location(s). Remove it from locations first.`,
      );
      return;
    }
  ...
  const toggleEquipmentForLocation = (equipmentId: string) => {
    setLocationForm((prev) => {
      ...
      if (current.includes(equipmentId)) {
        return {
          ...prev,
          equipmentIds: current.filter((id) => id !== equipmentId),
        };
      } else {
  ...
  const selectCatalogTelescope = (
    telescope: (typeof equipmentCatalog.telescopes)[0],
  ) => {
  ...
  const selectCatalogCamera = (
    camera: (typeof equipmentCatalog.cameras)[0],
  ) => {
    setEquipmentForm((prev) => ({
      ...prev,
      camera: {
              ...
              <CardContent>
                {locations.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p>
                      No locations configured. Add your first observing location
                      to get started.
                    </p>
                  </div>
                ) : (
                          ...
                          <div className="mb-2">
                            {horizonStats ? (
                              <div className="text-xs text-muted-foreground flex items-center gap-2">
                                ...
                                <span>
                                  {horizonStats.count} points, avg{" "}
                                  {horizonStats.avgAlt.toFixed(1)}°
                                </span>
                              </div>
                            ) : (
                              <div className="text-xs text-muted-foreground flex items-center gap-2">
                          ...
                          <div className="mb-3">
                            {loc.equipmentIds && loc.equipmentIds.length > 0 ? (
                              <div className="text-xs text-muted-foreground flex items-center gap-2">
                                ...
                                <span>
                                  {loc.equipmentIds.length} equipment set
                                  {loc.equipmentIds.length > 1 ? "s" : ""}
                                </span>
                              </div>
                            ) : (
                              <div className="text-xs text-muted-foreground flex items-center gap-2">
              ...
              <CardContent>
                {equipmentSets.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p>
                      No equipment configured. Add your first equipment set to
                      get started.
                    </p>
                  </div>
                ) : (
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {equipmentSets.map((eq) => {
                      const usedByCount = locations.filter((loc) =>
                        loc.equipmentIds?.includes(eq.id),
                      ).length;
                      return (
                        <div
                          key={eq.id}
                          ...
                          <div className="space-y-1 text-xs text-muted-foreground">
                            {eq.telescope && (
                              <div className="flex items-center gap-2">
                                ...
                                {eq.telescope.aperture && (
                                  <span className="opacity-60">
                                    ({eq.telescope.aperture}mm)
                                  </span>
                                )}
                              </div>
                            )}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>Solver</Label>
                <Select
                  value={plateSolveSolver}
                  onValueChange={savePlateSolveSolver}
                >
                  <SelectTrigger>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {plateSolveSolver === "local" &&
                    "Requires local astrometry.net installation with index files."}
                  {plateSolveSolver === "astap" &&
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        variant="outline"
                        onClick={saveLocalAstrometryUrl}
                      >
                        Save
                      </Button>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Leave empty to use the public nova.astrometry.net server.
                      Set this if you have a local astrometry.net instance
                      running (e.g., http://localhost:8080 or
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Local SQLite database information
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
            ...
            <Card className="md:col-span-2">
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-2">
                  ...
                  >
                    <RefreshCw
                      className={`w-4 h-4 mr-2 ${isLoadingBackups ? "animate-spin" : ""}`}
                    />
                    Refresh
                  </Button>
                </div>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mb-2">
                    Scan images for companion FITS files and update database
                    records. This links JPG preview images with their original
                    FITS data.
                  </p>
                  <Button
                  ...
                  >
                    <RefreshCw
                      className={`w-4 h-4 mr-2 ${isPopulatingFits ? "animate-spin" : ""}`}
                    />
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="pt-2 border-t">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Format: each line contains "azimuth altitude" in degrees (0-360,
      ...
      <Dialog open={restoreDialogOpen} onOpenChange={setRestoreDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to restore from this backup? The current
              database will be backed up before restoring.
            </DialogDescription>
          </DialogHeader>
          {selectedBackup && (
            <div className="p-3 bg-muted rounded-lg">
              ...
              <p className="text-xs text-muted-foreground">
                {formatSize(selectedBackup.size_bytes)} -{" "}
      ...
      <Dialog open={equipmentDialogOpen} onOpenChange={setEquipmentDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Define your imaging equipment configuration. All fields except
              name are optional.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-4">
            ...
            {!editingEquipmentId && (
              <div className="p-3 bg-muted/50 rounded-lg">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Choosing a preset will populate all fields below. You can then
                  customize as needed.
                </p>
              </div>
            )}
            ...
            <div className="pt-2 border-t">
                    ...
                    {filteredTelescopes.map((t, i) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground">
                          {t.aperture}mm f/
                          {(t.focalLength / t.aperture).toFixed(1)}
                        </span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <Label htmlFor="eq-telescope-name" className="text-xs">
                    Name
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="eq-telescope-aperture" className="text-xs">
                    Aperture (mm)
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="eq-telescope-focal" className="text-xs">
                    Focal Length (mm)
                  </Label>
                  <Input
                ...
                <div className="col-span-2">
                  <Label htmlFor="eq-telescope-type" className="text-xs">
                    Type
                  </Label>
                  <Input
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="eq-mount-name" className="text-xs">
                    Name
                  </Label>
                  <Input
                    id="eq-mount-name"
                ...
                <div>
                  <Label htmlFor="eq-mount-type" className="text-xs">
                    Type
                  </Label>
                  <Input
                    id="eq-mount-type"
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <Label htmlFor="eq-camera-name" className="text-xs">
                    Name
                  </Label>
                  <Input
                    id="eq-camera-name"
                ...
                <div>
                  <Label htmlFor="eq-camera-sensor-w" className="text-xs">
                    Sensor Width (mm)
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="eq-camera-sensor-h" className="text-xs">
                    Sensor Height (mm)
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="eq-camera-pixel" className="text-xs">
                    Pixel Size (microns)
                  </Label>
                  <Input
                    id="eq-camera-pixel"
                ...
                <div>
                  <Label htmlFor="eq-camera-resolution" className="text-xs">
                    Resolution
                  </Label>
                  <Input
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="eq-guide-name" className="text-xs">
                    Name
                  </Label>
                  <Input
                    id="eq-guide-name"
                ...
                <div>
                  <Label htmlFor="eq-guide-aperture" className="text-xs">
                    Aperture (mm)
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="eq-guide-focal" className="text-xs">
                    Focal Length (mm)
                  </Label>
                  <Input
                    id="eq-guide-focal"
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="eq-guidecam-name" className="text-xs">
                    Name
                  </Label>
                  <Input
                    id="eq-guidecam-name"
                ...
                <div>
                  <Label htmlFor="eq-guidecam-pixel" className="text-xs">
                    Pixel Size (microns)
                  </Label>
                  <Input
